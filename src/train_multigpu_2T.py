...
def train(gpuid,
    ...
    trainloader, valloader = compile_data(version, dataroot, data_aug_conf=cfg.data_aug_conf,
                                          ...
                                          train_label = cfg.train_label,dist=True, rank = gpuid, sw = sw)   
    ...
    dir_name = logdir + '/'
    model_dir = sorted( filter( os.path.isfile, glob.glob(dir_name + 'model*') ) ,reverse=True)
    ...
    opt_dir = sorted(filter( os.path.isfile, glob.glob(dir_name + 'opt*') ),reverse=True)

    continue_opt = os.path.exists(opt_dir[0]) if opt_dir else False
    ...
    if continue_training :
        ...
        if continue_opt:
            print('Loading last checkpoint ... ',opt_dir[0]) 
            opt_chk = torch.load(opt_dir[0])
            opt.load_state_dict(opt_chk['optimizer_state_dict'])        
            loss = opt_chk['loss']  
            counter = opt_chk['counter']  
    ...
    for epoch in range(lepochs,nepochs):
        ...
        for batchi, (imgs, rots, trans, intrins, post_rots, post_trans, binimgs, points) in enumerate(trainloader):
            ...
            if counter % val_step == 0 and gpuid == 0:
                ...
                optname = os.path.join(logdir, "opt.pt")
                torch.save({
